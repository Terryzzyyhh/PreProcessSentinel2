...
class L2A_SceneClass(object):
    ...
    def preprocess(self):
        ...
        CM = self.classificationMask
        if self.config.qltmask == True and self.tables.hasBand(self.tables.QMS) == True :
    ...
    def L2A_SnowPostProcessingCCI(self):
        if self.tables.hasBand(self.tables.SNC) == True:
            ...
            if SNC[SNC != 254].sum() > 0:  # Enter only if tile is not full of sea pixels
                if SNC[SNC != 254].mean() < 10.0:
                    CM[(SNC == 254) & (CM == self._snowIce) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._medProbaClouds    # water
        return
    ...
    def L2A_CSND_5_2(self):
        ...
        CM[(R_B02_B11_GT_T22_R_B02_B11 == True) & (CM == self._notClassified) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._water
    ...
    def L2A_CSND_7(self):
        ...
        if (self.tables.hasBand(self.tables.DEM) == True):
            ...
            CM[(B10 > T1_B10) & (B10 < T2_B10) & (B02 < REFL_BLUE_MAX) & (dem < T_dem) & (CMC < T_CLOUD_MP) & \
               (CM!=self._noData) & (CM!=self._saturatedDefective)] = CIR
        else:
            CM[(B10 > T1_B10) & (B10 < T2_B10) & (B02 < REFL_BLUE_MAX) & (CMC < T_CLOUD_MP) & \
               (CM!=self._noData) & (CM!=self._saturatedDefective)] = CIR
        
        self.logger.debug(statistics(CMC[(CM == CIR)], 'CM THIN_CIRRUS'))
        #CM[(B10 >= T2_B10) & (CM == self._notClassified)]= MPC
        CM[(B10 >= T2_B10) & (CMC < T_CLOUD_HP) & \
           (CM!=self._noData) & (CM!=self._saturatedDefective)] = MPC
        # assign medium probability clouds class to Thick cirrus >= T2_B10
    ...
    def L2A_CSHD_2(self):
        ...
        # @ Jerome: check if this fix can be applied for situations
        # when y_aa < distr_shad.size in testmode:
            filt_b = zeros([distr_shad.size, x_aa], float32)
            filt_b[0:distr_shad.size, 0] = distr_shad
        ...
        cloud_mask_aa = zeros([filt_b.shape[0], x_aa], float32) # to avoid FFT aliasing
        ...
        if (y != self.confidenceMaskCloud.shape[0]) | (x != self.confidenceMaskCloud.shape[1]):
            ...
            cloud_mask_aa[0:y, 0:x] = copy(tmp_cloud_mask[:, :])
        else:
        ...
        CM = self.classificationMask
        shadow_prob[(CM==self._noData) | (CM==self._saturatedDefective)] = 0.0
        return shadow_prob
    ...
    def L2A_DarkVegetationRecovery(self):
        ...
        CM[F2 & (CM == self._darkFeatures) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._vegetation
        CM[F2 & (CM == self._notClassified) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._vegetation
        return
    ...
    def L2A_WaterPixelRecoveryCCI(self):
        if (self.tables.hasBand(self.tables.WBI) == True):
            ...
            CM = self.classificationMask
            CM[(WBI == 2) & (CM == self._darkFeatures) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._water
            CM[(WBI == 2) & (CM == self._cloudShadows) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._water
            CM[(WBI == 2) & (CM == self._notClassified) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._water
            CM[(WBI == 2) & (CM == self._lowProbaClouds) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._water
        return
    ...
    def L2A_WaterPixelCleaningwithDEM(self):
        ...
        if (self.tables.hasBand(self.tables.DEM) == True):
            ...
            CM = self.classificationMask
            CM[clean_area & (CM == self._water) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._darkFeatures
        return
    ...
    def L2A_CloudShadowPixelCleaningwithDEM(self):
        ...
        if (self.tables.hasBand(self.tables.DEM) == True):
            ...
            CM = self.classificationMask
            CM[clean_area & (CM == self._cloudShadows) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._darkFeatures
        return
...
#        return 
    ...
    def L2A_TopographicShadowwithDEM(self):
        ...
        if (self.tables.hasBand(self.tables.DEM) == True) & (self.tables.hasBand(self.tables.WBI) == True):
            ...
            CM[clean_area & F2 & potential_pixels & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._darkFeatures
        return
    ...
    def L2A_SnowRecovery(self):
        ...
        struct = iterate_structure(generate_binary_structure(2,1), 3)
        snow_mask_dil = binary_dilation(snow_mask, struct)
        CM[snow_mask_dil & (B11 < B03) & (CM == self._notClassified) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._snowIce
        return
    ...
    def L2A_SoilRecovery(self):
        ...
        CM = self.classificationMask
        CM[F4 & (CM == self._darkFeatures) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils
    ...
    def L2A_LandPixelRecoveryB10B09B8A(self):
        if (self.tables.hasBand(self.tables.LCM) == True):
            ...
            CM[(LCM != 210) & ((CM == self._medProbaClouds) | (CM == self._highProbaClouds)) & \
               (B10 < T_B10) & (Ratio_B09_B8A < T_Ratio_B09_B8A) & \
               (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils  # all land pixels
        return
    ...
    def L2A_CirrusPixelRecoveryB10(self):
        if (self.tables.hasBand(self.tables.LCM) == True):
            ...
            if (self.tables.hasBand(self.tables.DEM) == True):
                ...
                CM[(CM == self._water) & (B10 > T2_B10_Water) & (dem < T_dem) & \
                   (CM != self._medProbaClouds) & (CM != self._highProbaClouds) & \
                   (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._thinCirrus
            else:
                ...
                CM[(CM == self._water) & (B10 > T2_B10_Water) & (CM != self._medProbaClouds) & \
                   (CM != self._highProbaClouds) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._thinCirrus
        return
    ...
    def L2A_UrbanBarePixelRecoveryCCI(self):
        if (self.tables.hasBand(self.tables.LCM) == True):
            ...
            CM[(LCM == 190) & (CM == self._notClassified) & \
               (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils    # Urban class = 190

            # ESA CCI Urban pixel recovery step 2 for lowProbaClouds and medProbaClouds
            UrbanMask = ((LCM == 190) & ((CM == self._lowProbaClouds) | (CM == self._medProbaClouds)) & \
                (CM!=self._noData) & (CM!=self._saturatedDefective)).astype('uint8')
            T_urban = 35.0
            Label_UrbanMask = self.L2A_FineMorphoRecovery(UrbanMask, CM, T_urban)
            CM[(Label_UrbanMask != 0) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils

            # ESA CCI Bare pixel recovery step 1 for notClassified pixels
            CM[(LCM >= 200) & (LCM <= 202) & (CM == self._notClassified) & \
               (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils    # Bare classes = 200, 201, 202

            # ESA CCI Bare pixel recovery step 2 for lowProbaClouds and medProbaClouds
            BrightMask = ((LCM >= 200) & (LCM <= 202) & ((CM == self._lowProbaClouds) | (CM == self._medProbaClouds)) & \
                (CM!=self._noData) & (CM!=self._saturatedDefective)).astype('uint8')
            T_bright = 35.0
            Label_BrightMask = self.L2A_FineMorphoRecovery(BrightMask, CM, T_bright)
            CM[(Label_BrightMask != 0) & (CM!=self._noData) & (CM!=self._saturatedDefective)] = self._bareSoils
        return
