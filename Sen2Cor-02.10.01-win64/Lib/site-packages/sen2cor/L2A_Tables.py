...
class L2A_Tables(object):
    ...
    def create_lost_band_list(self):
        ...
        if not data_list_mask:
            self.config.timestamp('L2A_Tables: *GENERAL_QUALITY* file not found.')
            return None
        else:
        ...
        if inspection_status == 'FAILED':
            ...
            for i in range(0, len(list)):
                ...
                list[i] = detected_mask[0]

                # list[i] = 'MSK_QUALIT_{}.jp2'.format(list[i])
            return list
        else:
            return None
    ...
    def gdalDEM_copernicus(self):
        ...
        try:  # does the list of missing tiles exist?
            with open(os.path.join(self.config.aux_dir, copernicus_list)) as missing_tiles:
                self.missing_tiles_list = [line.rstrip('\n') for line in missing_tiles]
    ...
    def importBandImg(self, index, filename):
        ...
        if self.config.productVersion > float32(14.6):
            ...
            # to exclude background values from offset subtracttion,
    ...
    def resampleBand(self, index, indataArr):
        ...
        if (src_nrows / tgt_nrows) == 2:
            ...
            if index in [14, 17, 18, 19]:
                ...
                return (skit_resize(indataArr.astype(uint16), ([size_up_nrows, size_up_ncols]), order=1) * 65535.).round().astype(uint16)
            else:
            ...
            try:
                ...
                if bandName != 'TCI':
                    if self.config.raw:
                        ...
                        MEM_dataset.BuildOverviews(resampling="MODE", overviewlist=(2, 4, 8, 16))
                        kwargs = "-co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co GDAL_TIFF_OVRE_BLOCKSIZE=1024 -co BLOCKXSIZE=1024 -co BLOCKYSIZE=1024 -co COMPRESS=LZW"
