...
class L2A_Quality(object):
    def __init__(self, config, tables):
        ...
        self._nr_px_total = sc.size
        self._nr_px_no_data = sc[sc==config.noData].size
        self._nr_px_saturated_defective = sc[sc==config.saturatedDefective].size
        self._nr_px_valid = self._nr_px_total - self._nr_px_no_data - self._nr_px_saturated_defective
    ...
    def update_qi(self):
        switch_case = {
            ...
            'NODATA_PIXEL_PERCENTAGE': format('%f' % self.get_percentage\
                (self._config.noData, self._nr_px_no_data)),
